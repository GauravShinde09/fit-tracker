...
//           `http://localhost:5000/api/workouts/history?date=${date}`
//         );
...
//   // ✅ CALORIE CALCULATION
//   // ✅ CALORIE CALCULATION (Realistic MET-based formula)
// const calculateCalories = (sets, duration) => {
//   const bodyWeight = 63; // your weight in kg
//   const MET = 4.5; // moderate weight training (3.5–6 range)

//   const minutes = Number(duration) || 0;

//   const calories = (MET * bodyWeight * minutes) / 60;
...
//     // ✅ DO NOT manually update workouts state
//     setEditingId(null);
//     setEditData(null);
...
//       {loading ? (
//         <p className="loading-message">Fetching workouts...</p>
//       ) : workouts.length === 0 ? (
//         <p>No workouts logged yet for {date}.</p>
//       ) : (
//         <ul>
//           {workouts.map((workout, index) => (
//             <li key={workout._id || index} className="workout-item">
...
//                   <p>
//                     Calories:{" "}
//                     {calculateCalories(editData.sets, editData.duration)} kcal
//                   </p>
...
//       <p key={i}>
//         Set {i + 1} — Weight: {set?.weight || 0}kg | Reps: {set?.reps || 0}
//       </p>
//     ))
//   ) : (
...
const WorkoutHistory = ({ date, refreshTrigger }) => {
  ...
  useEffect(() => {
    ...
    const fetchWorkouts = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/workouts/history?date=${date}`, {
          headers: {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const token = localStorage.getItem("userToken");
      const response = await fetch(`http://localhost:5000/api/workouts/${editingId}`, {
        method: "PUT",
        headers: { 
